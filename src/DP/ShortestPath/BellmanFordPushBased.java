...
public class BellmanFordPushBased {

    // push based! O(VE) worst case, O(V)space!
    ...
    public static void bellmanFord(int V, int E, ArrayList<Edge> edges, int s, int t) {
        ...
        for(int i = 0; i < V - 1; i++) {
            boolean updatedAtThisIteration = false;
            for(int j = 0; j < E; j++) {
                int u = edges.get(j).from;
                int v = edges.get(j).to;
                int cost = edges.get(j).cost;
                // if dist[u] has been updated in previous iteration
                if (dist[u] != Integer.MAX_VALUE) {
                    // check if d[u] + cvw < dist[v], if yes update dist[v] (u -> v)
                    if (dist[u] + cost < dist[v]) {
                        dist[v] = dist[u] + cost;
                        pred[v] = u;
                        updatedAtThisIteration = true;
                    }
                }
            }
            //If no value changed in this iteration, then end the algorithm and return dist[t]
            if (!updatedAtThisIteration) break;
        }
    ...
    public static void getPath(int[] pred, int t) {
